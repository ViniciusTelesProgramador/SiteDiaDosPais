...
export async function GET(
  ...
  const { data, error } = await supabaseAdmin
    .from('paginas')
    .select(
      'id, slug, email_comprador, nome_destinatario, mensagem, blocos, midias, tema, plano, pago, revelar_em, reacao_emoji, reacao_texto, musica_youtube_id, audio_url, visualizacoes, criado_em'
    )
    .eq('id', params.id)
    .single();
